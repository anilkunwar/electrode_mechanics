...
Date: 2024
License: MIT
VERSION 2.7.1 - CRITICAL UPDATE:
1. ✅ UPDATED Stiffness Coefficients - Full elastic tensor C_ij from validated literature
   - BCT Sn: C₁₁=72.0, C₁₂=58.5, C₁₃=37.4, C₃₃=88.0, C₄₄=21.9, C₅₅=21.9, C₆₆=24.0 GPa [Jiang2019-JEM]
   - Li₂Sn₅: C₁₁=60.08, C₁₂=55.75, C₁₃=19.55, C₃₃=97.48, C₄₄=20.44, C₅₅=20.44, C₆₆=14.93 GPa [ZHANG2016111]
2. ✅ FIXED Calculator Method Availability - Proper ASE Calculator interface
3. ✅ REAL DFT E-V MAPPING - Correct Sn BCT structure with V₀ ≈ 108.19 Å³
4. ✅ CORRECT CRYSTAL STRUCTURES - ASE crystal() with validated Wyckoff positions
- β-Sn: I4₁/amd (#141), V₀ = 108.19 Å³ (4 atoms), Wyckoff 4a (0,0,0)
- Li₂Sn₅: P4/mbm (#127), V₀ = 337.44 Å³ (14 atoms), Wyckoff 2a+8j+4g
5. ✅ streamlit-molstar INTEGRATION - Cloud-compatible 3D CIF visualization
6. ✅ FIXED CIF EXPORT - BytesIO buffer + ASE symmetry handling
7. ✅ FORCE_REAL_DFT OPTION - Hard enforcement with clear error messaging
8. ✅ STRUCTURE VALIDATION - Min distance checks to catch unphysical overlaps
9. ✅ FIXED Calculator Attachment - Methods accessible via atoms.calc
...
ELASTIC STIFFNESS TENSOR COMPONENTS $C_{ijkl}$ (GPa) FOR BCT Sn & Li₂Sn₅:
=========================================================================
Table 1: Elastic Stiffness Tensor Components $C_{ijkl}$ (GPa) for BCT Sn ~\cite{Jiang2019-JEM} Li$_2$Sn$_5$ ~\cite{ZHANG2016111} phases.
Material | C11    | C12    | C13    | C22    | C23    | C33    | C44    | C55    | C66
---------|--------|--------|--------|--------|--------|--------|--------|--------|-------
BCT Sn   | 72.0   | 58.5   | 37.4   | 72.0   | 37.4   | 88.0   | 21.9   | 21.9   | 24.0
Li₂Sn₅   | 60.08  | 55.75  | 19.55  | 60.08  | 19.55  | 97.48  | 20.44  | 20.44  | 14.93
...
AR > 1: c-axis stiffer (observed in both Sn and Li₂Sn₅ per updated literature values)
...
st.set_page_config(
    ...
    menu_items={
        ...
        **Version**: 2.7.1 (Updated Stiffness Coefficients + Real DFT + Correct Structures + streamlit-molstar)
        **License**: MIT
        """
    }
)
...
| 🔹 Phase 3 | Anisotropic Elasticity | C₁₁, C₃₃, Full Stiffness Tensor, Anisotropy Ratio AR | 3-60 min (REAL DFT) |
...
**🔬 Crystallographic & Mechanical Validation**:
...
- Updated Elastic Constants (GPa) from Jiang et al. (2019) & Zhang et al. (2016)
""")
...
# ============================================================================
# DEMO DATA GENERATORS (WITH CORRECTED VOLUMES & UPDATED STIFFNESS COEFFICIENTS)
# ============================================================================
...
def get_demo_elasticity_results(structure_name, v0):
    ...
    UPDATED v2.7.1: Correct stiffness coefficients from literature:
    - BCT Sn: Jiang et al., JEM (2019) -> C11=72.0, C33=88.0 GPa
    - Li2Sn5: Zhang et al., J. Alloys Compd. (2016) -> C11=60.08, C33=97.48 GPa
    """
    demo_params = {
        'Sn': {
            'c11': 72.0, 'c12': 58.5, 'c13': 37.4, 'c22': 72.0, 'c23': 37.4,
            'c33': 88.0, 'c44': 21.9, 'c55': 21.9, 'c66': 24.0
        },
        'Li2Sn5': {
            'c11': 60.08, 'c12': 55.75, 'c13': 19.55, 'c22': 60.08, 'c23': 19.55,
            'c33': 97.48, 'c44': 20.44, 'c55': 20.44, 'c66': 14.93
        }
    }
    if structure_name not in demo_params:
        return None
...
def compute_reference_energies(ecut, kpts, fmax, convergence_energy=1e-5, convergence_density=1e-4,
    ...
    🔧🔧🔧 FIXED v2.7.0: Proper calculator attachment before optimization
    """
...
def compute_ev_curve(structure_name, a_init, c_init, symbols, spacegroup, basis,
    ...
    🔧🔧🔧 FIXED v2.7.0: Proper demo vs real DFT separation + correct structures
    ...
    try:
        if structure_name == 'Sn (BCT)':
            template = get_beta_sn_structure(a=a_init, c=c_init)
        elif structure_name == 'Li2Sn5':
            template = get_li2sn5_structure(a=a_init, c=c_init)
        else:
...
def compute_anisotropic_elasticity(structure_name, a0, c0, symbols, spacegroup, basis,
    ...
    try:
        if structure_name == 'Sn':
            ...
            template = get_li2sn5_structure(a=a0, c=c0)
        else:
...
with tab3:
    ...
    with st.expander("📚 Methodology", expanded=False):
        ...
        **Updated Stiffness Coefficients (GPa)**:
        - BCT Sn: C₁₁=72.0, C₃₃=88.0 [Jiang2019-JEM]
        - Li₂Sn₅: C₁₁=60.08, C₃₃=97.48 [ZHANG2016111]
        """)
    if not GPAW_AVAILABLE:
    ...
    if sn_el is not None:
        ...
        with col3:
            ...
            st.metric("Anisotropy AR", f"{ar_val:.3f}",
                      delta="c-stiff" if ar_val > 1 else "isotropic" if 0.9 <= ar_val <= 1.1 else "c-soft")
        ...
        if st.session_state.phase_results['phase4'] is not None:
            ...
            with col2:
                ...
                st.metric("Anisotropy Ratio AR", f"{ar_val:.3f}",
                          delta="c-stiff" if ar_val > 1 else "isotropic")
            with col3:
    ...
    if not (sn_eos and li_eos):
        ...
        with col3:
            if li_el:
                ...
                <div style='font-size: 0.9rem; opacity: 0.9'>{'c-stiff' if ar>1 else 'isotropic'}</div>
                </div>
                """, unsafe_allow_html=True)
            else:
...
with tab6:
    ...
    **Crystallographic Data**:
    ...
    @st.cache_resource
    def get_structures():
        sn = get_beta_sn_structure()
        li = get_li2sn5_structure()
        return sn, li
...
# App Version: 2.7.1 (Updated Stiffness Coefficients + Real DFT + Cloud Visualization)
## β-Sn (BCT)
- Space Group: I4₁/amd (#141)
...
with st.expander("❓ Help & Frequently Asked Questions", expanded=False):
    ...
    **Q: How do I interpret the results?**
    ...
    - AR > 1: c-axis stiffer than basal plane (observed in both phases per updated literature)
    ...
    - Jiang, X. et al. (2019). Elastic properties of BCT Sn. *Journal of Electronic Materials* [Jiang2019-JEM].
    - Zhang, Y. et al. (2016). Mechanical behavior of Li₂Sn₅. *Journal of Alloys and Compounds* [ZHANG2016111].
...
<em>Version 2.7.1 | Updated Stiffness Coefficients + Real DFT + Correct Structures + Cloud Visualization</em><br>
