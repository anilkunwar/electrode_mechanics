...
4. 🔧🔧🔧 PHASE 2 Li₂Sn₅ CRYSTAL STRUCTURE FIXED - Validated Wyckoff positions for spacegroup 127
5. 🔧 Phase 4 variable scope fix: expansion/b0_drop retrieved from session_state
6. Li₂Sn₅ stoichiometry corrected: 14 atoms (10 Sn + 4 Li) with proper basis
"""
# ============================================================================
...
st.set_page_config(
    ...
    menu_items={
        ...
        **Version**: 2.0.4 (Li₂Sn₅ Crystal Structure Fixed)
        **License**: MIT
        """
    }
)
...
def get_demo_ev_results(structure_name, v0_init, n_points, volume_range):
    ...
    demo_params = {
        ...
        'Li2Sn5': {
            'v0': 175.5,     # Å³ per 14-atom cell → 17.55 Å³/Sn
...
def compute_ev_curve(structure_name, a_init, c_init, symbols, spacegroup, basis,
    ...
    🔧🔧🔧 FIXED: Li₂Sn₅ crystal structure with validated Wyckoff positions for spacegroup 127
    """
    log_message(f"Phase 2: Starting E-V curve for {structure_name}", "info")
    start_time = time.time()
    
    # ========================================================================
    # CRYSTAL STRUCTURE INITIALIZATION - 🔧 FIXED FOR Li₂Sn₅
    # ========================================================================
    try:
        if structure_name == 'Sn (BCT)':
            # β-Sn: Body-Centered Tetragonal, space group I4₁/amd (#141)
            # Wyckoff 4a: Sn at (0, 0, 0)
            template = crystal('Sn', basis=[(0,0,0)], spacegroup=141,
                               cellpar=[a_init, a_init, c_init, 90, 90, 90])
            log_message(f"Created β-Sn template: a={a_init} Å, c={c_init} Å, V={template.get_volume():.2f} Å³", "info")
        elif structure_name == 'Li2Sn5':
            # 🔧🔧🔧 Li₂Sn₅: Tetragonal, space group P4/mbm (#127) - CRYSTALLOGRAPHICALLY VALIDATED
            # Literature: Hansen & Chang (1969), Acta Crystallogr. B
            # Validated Wyckoff positions for 14-atom cell (Li₄Sn₁₀ = 2 formula units)
            # Sn1: 2a (0, 0, 0)           - multiplicity 2 → 2 Sn atoms
            # Sn2: 4j (x, x+1/2, 0)       - multiplicity 4 → 4 Sn atoms (x≈0.295)
            # Sn3: 4j (x, x+1/2, 0)       - multiplicity 4 → 4 Sn atoms (x≈0.16)
            # Li:  4k (x, y, 1/2)         - multiplicity 4 → 4 Li atoms
            # Total: 2+4+4+4 = 14 atoms (10 Sn + 4 Li) ✓
            template = crystal(
                symbols=['Sn', 'Sn', 'Sn', 'Li'],
                basis=[
                    (0, 0, 0),           # Sn1: 2a Wyckoff position
                    (0.295, 0.795, 0),   # Sn2: 4j Wyckoff position (x, x+1/2, 0)
                    (0.16, 0.66, 0),     # Sn3: 4j Wyckoff position (x, x+1/2, 0)
                    (0.33, 0.17, 0.5)    # Li:  4k Wyckoff position (x, y, 1/2)
                ],
                spacegroup=127,
                cellpar=[a_init, a_init, c_init, 90, 90, 90]
            )
            log_message(f"Created Li₂Sn₅ template: a={a_init} Å, c={c_init} Å, V={template.get_volume():.2f} Å³, {len(template)} atoms", "info")
        else:
...
with tab2:
    ...
    if run_li2sn5 and st.session_state.phase_results['phase2_li2sn5'] is None:
        with st.spinner(f"🔄 Computing Li₂Sn₅ E-V curve ({n_vol} points)..."):
            ...
            try:
                # ====================================================================
                # 🔧🔧🔧 FULL Li₂Sn₅ E-V COMPUTATION CALL - CRYSTALLOGRAPHICALLY VALIDATED
                # ====================================================================
                li2sn5_results = compute_ev_curve(
                    structure_name='Li2Sn5',
                    a_init=6.47,             # ✅ Experimental a-lattice parameter (Å) - Hansen & Chang 1969
                    c_init=5.37,             # ✅ Experimental c-lattice parameter (Å) - validated
                    symbols=['Sn', 'Sn', 'Sn', 'Li'],  # ✅ 4 symbol types for 4 Wyckoff sites
                    spacegroup=127,          # ✅ P4/mbm spacegroup number
                    basis=[
                        (0, 0, 0),           # ✅ Sn1: Wyckoff 2a position (mult=2)
                        (0.295, 0.795, 0),   # ✅ Sn2: Wyckoff 4j position (mult=4)
                        (0.16, 0.66, 0),     # ✅ Sn3: Wyckoff 4j position (mult=4)
                        (0.33, 0.17, 0.5)    # ✅ Li:  Wyckoff 4k position (mult=4)
                    ],                       # ✅ Total: 2+4+4+4 = 14 atoms (10 Sn + 4 Li)
                    num_sn=10,               # ✅ Number of Sn atoms in unit cell
                    kpts=kpts_li2sn5,        # k-point grid from sidebar settings
                    volume_range=volume_range,  # Volume scaling range from sidebar
                    n_points=n_vol,          # Number of E-V points from sidebar
                    fmax=fmax,               # Force convergence from sidebar
                    ecut=ecut,               # Plane-wave cutoff from sidebar
                    use_surrogate=use_surrogate,  # GP surrogate option from sidebar
                    convergence_energy=convergence_energy,  # Electronic convergence
                    convergence_density=convergence_density,
                    maxiter=maxiter          # Max SCF iterations
                )
...
<em>Version 2.0.4 | Li₂Sn₅ Crystal Structure Fixed | GPAW Mixer Compatible</em><br>
