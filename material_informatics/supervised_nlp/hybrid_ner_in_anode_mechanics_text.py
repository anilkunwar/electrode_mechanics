...
def create_entity_prob_plot(prob_dict, entity_text):
    ...
    try:
        if not prob_dict:
            logger.warning(f"No probabilities for {entity_text} plot.")
            update_log(f"No probabilities for {entity_text} plot.")
        ...
        ax.set_title(f"Estimated Probabilities for '{entity_text}'", fontsize=st.session_state.title_font_size, pad=15, weight='bold')
        ...
        logger.info(f"Probability plot for {entity_text} generated successfully.")
        update_log(f"Probability plot for {entity_text} generated successfully.")
        return buf_png, buf_svg
    
    except Exception as e:
        logger.error(f"Error creating probability plot for {entity_text}: {e}")
        update_log(f"Error creating probability plot for {entity_text}: {e}")
        return BytesIO(), BytesIO()
...
- The plots below visualize these probabilities for each entity.
"""
    return explanation
...
def main():
    ...
    It also demonstrates a hybrid approach combining both methods, prioritizing domain-specific rules for LiₓSnᵧ phases and anode keywords. Visualizations include a heatmap and entity type counts.
    ...
    with tab5:
        ...
        st.markdown("Estimated probabilities for each spaCy-detected entity, with bar plots and mathematical explanation.")
        ...
        if "entity_probs" not in st.session_state or not st.session_state.entity_probs:
            ...
            for entity_text, probs in st.session_state.entity_probs.items():
                st.write(f"**Probability Plot for '{entity_text}'**")
                prob_png, prob_svg = create_entity_prob_plot(probs, entity_text)
                if prob_png.getvalue():
                    st.image(prob_png, caption=f"Estimated Probabilities for '{entity_text}'", use_column_width=True)
                    st.download_button(
                        label=f"Download Probability Plot for '{entity_text}' (PNG)",
                        data=prob_png,
                        file_name=f"prob_plot_{entity_text.lower().replace(' ', '_')}.png",
                        mime="image/png"
                    )
                    st.download_button(
                        label=f"Download Probability Plot for '{entity_text}' (SVG)",
                        data=prob_svg,
                        file_name=f"prob_plot_{entity_text.lower().replace(' ', '_')}.svg",
                        mime="image/svg+xml"
                    )
                else:
                    st.warning(f"Failed to generate probability plot for '{entity_text}'. Check the 'Logs' tab.")
