...
def count_terms(text, terms):
    """Count occurrences of specified terms in the text."""
    term_counts = Counter()
...
def create_network(df, phase_counts, layout_algorithm="fruchterman_reingold", phase_shape="s", mech_term_shape="d", edge_style="solid", label_font_size=12, edge_thickness=1.0):
    """Create network graph with fixed node sizes for publication quality."""
    G = nx.Graph()
    ...
    try:
        ...
        plt.title("Network Analysis of LiₓSnᵧ Relations with Mechanical Terminology (Fixed Size)", fontsize=20, pad=40, weight="bold")
        ...
        logger.error(f"Error creating fixed-size network: {e}")
        update_log(f"Error creating fixed-size network: {e}", tab="analysis")
        return BytesIO(), BytesIO(), []

def create_network_count_dependent(df, phase_counts, layout_algorithm="fruchterman_reingold", phase_shape="s", mech_term_shape="d", edge_style="solid", label_font_size=12, edge_thickness=1.0, phase_node_size_scale=500, mech_term_node_size_scale=500):
    """Create network graph with count-dependent node sizes for publication quality."""
    G = nx.Graph()
    ...
    try:
        ...
        # Count mechanical term occurrences
        term_counts = count_terms(text, all_terms)
        logger.info(f"Mechanical term counts: {dict(term_counts)}")
        update_log(f"Mechanical term counts: {dict(term_counts)}", tab="analysis")
            ...
            try:
                for phase in all_phases:
                    ...
                    for term in all_terms:
                        ...
                        if phase_lower in sentence_lower and term_lower in sentence_lower:
                            co_occurrences[(phase, term)] += 1
                update_log(f"Processed sentence {i+1}/{len(sentences)} for count-dependent network analysis.", tab="analysis")
                progress_bar.progress((i + 1) / len(sentences))
            except Exception as e:
                logger.warning(f"Error processing sentence {i+1}: {e}")
                update_log(f"Error processing sentence {i+1}: {e}", tab="analysis")
        ...
        for node in G.nodes:
            ...
            node_type = G.nodes[node]["type"]
            community = community_map.get(node, -1)
            count = phase_counts[node] if node_type == "phase" else term_counts.get(node, 0)
            network_data.append({"Node": node, "Type": node_type, "Degree": degree, "Community": community, "Count": count})
        for node1, node2, data in G.edges(data=True):
            network_data.append({"Node": f"{node1}-{node2}", "Type": "Edge", "Weight": data["weight"], "Community": -1, "Count": 0})
        ...
        for node in G.nodes:
            if G.nodes[node]["type"] == "phase":
                node_colors.append("#FF9999")
                count = phase_counts.get(node, 1)
                # Scale size logarithmically to avoid extreme sizes
                node_sizes.append(phase_node_size_scale * (1 + np.log1p(count)))
                node_shapes.append(phase_shape)
            elif G.nodes[node]["type"] == "mechanical":
                node_colors.append("#66CC66")
                count = term_counts.get(node, 1)
                node_sizes.append(mech_term_node_size_scale * (1 + np.log1p(count)))
                node_shapes.append(mech_term_shape)
        ...
        plt.title("Network Analysis of LiₓSnᵧ Relations with Mechanical Terminology (Count-Dependent Size)", fontsize=20, pad=40, weight="bold")
        plt.grid(True, linestyle="--", alpha=0.3)
        ...
        logger.error(f"Error creating count-dependent network: {e}")
        update_log(f"Error creating count-dependent network: {e}", tab="analysis")
        return BytesIO(), BytesIO(), []
...
def main():
    ...
    with tab2:
        ...
        with st.sidebar:
            ...
            colors = [
                "blue", "red", "green", "purple", "orange", "black", "gray", "white", "yellow", "brown",
                "pink", "cyan", "magenta", "lime", "teal", "navy", "maroon", "olive", "gold", "silver",
                "aqua", "indigo", "violet", "turquoise", "coral", "salmon", "chocolate", "crimson", "khaki",
                "orchid", "plum", "tan", "tomato", "wheat", "lavender", "seagreen", "skyblue", "slateblue",
                "dodgerblue", "hotpink", "darkorange", "darkgreen", "darkred", "darkblue", "lightblue",
                "lightgreen", "lightcoral", "lightsalmon", "lightseagreen", "lightpink", "lightgray",
                "deepskyblue", "firebrick", "forestgreen", "midnightblue", "sienna", "thistle", "Jet",
                "Rainbow", "Viridis", "Cividis", "Plasma", "Inferno", "Magma", "Turbo", "IceFire",
                "Picnic", "Portland", "Earth", "Electric", "Bluered", "Greens", "Greys", "YlGnBu"
            ]
            ...
            network_label_font_size = st.slider("Network label font size", min_value=10, max_value=100, value=22, step=1, key="network_label_font_size")
            #edge_thickness = st.slider("Edge thickness multiplier", min_value=0.5, max_value=5.0, value=1.5, step=0.1, key="edge_thickness")
            edge_thickness = st.slider("Edge thickness multiplier", min_value=0.5, max_value=10.0, value=1.5, step=0.1, key="edge_thickness")
        ...
        if db_file:
            try:
                ...
                st.subheader("Network Analysis of LiₓSnᵧ Relations with Mechanical Terminology (Fixed Size)")
                ...
                if network_buf_png.getvalue():
                    st.image(network_buf_png, caption="Network Graph of LiₓSnᵧ Relations with Mechanical Terminology (Fixed Size)", use_column_width=True)
                    st.download_button(
                        label="Download Fixed-Size Network Graph (PNG)",
                        data=network_buf_png,
                        file_name="network_graph_fixed.png",
                        mime="image/png"
                    )
                    st.download_button(
                        label="Download Fixed-Size Network Graph (SVG)",
                        data=network_buf_svg,
                        file_name="network_graph_fixed.svg",
                        mime="image/svg+xml"
                    )
                else:
                    st.warning("Failed to generate fixed-size network graph. Check logs for details.")
                
                st.subheader("Network Analysis of LiₓSnᵧ Relations with Mechanical Terminology (Count-Dependent Size)")
                network_count_buf_png, network_count_buf_svg, network_count_data = create_network_count_dependent(
                    df, final_phase_counts, layout_algorithm=layout_algorithm,
                    phase_shape=phase_shape, mech_term_shape=mech_term_shape,
                    edge_style=edge_style, label_font_size=network_label_font_size,
                    edge_thickness=edge_thickness, phase_node_size_scale=phase_node_size_scale,
                    mech_term_node_size_scale=mech_term_node_size_scale
                )
                if network_count_buf_png.getvalue():
                    st.image(network_count_buf_png, caption="Network Graph of LiₓSnᵧ Relations with Mechanical Terminology (Count-Dependent Size)", use_column_width=True)
                    st.download_button(
                        label="Download Count-Dependent Network Graph (PNG)",
                        data=network_count_buf_png,
                        file_name="network_graph_count_dependent.png",
                        mime="image/png"
                    )
                    st.download_button(
                        label="Download Count-Dependent Network Graph (SVG)",
                        data=network_count_buf_svg,
                        file_name="network_graph_count_dependent.svg",
                        mime="image/svg+xml"
                    )
                else:
                    st.warning("Failed to generate count-dependent network graph. Check logs for details.")
                
                st.subheader("Network Quantification (Count-Dependent)")
                if network_count_data:
                    network_df = pd.DataFrame(network_count_data)
                    ...
                    st.download_button(
                        label="Download Count-Dependent Network Quantification (CSV)",
                        data=network_csv_buffer,
                        file_name="network_quantification_count_dependent.csv",
                        mime="text/csv"
                    )
                else:
