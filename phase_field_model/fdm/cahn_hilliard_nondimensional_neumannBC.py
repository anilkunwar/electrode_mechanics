...
j0_dim = st.sidebar.number_input("Exchange current density (A/m²)", min_value=1e5, max_value=1e7, value=1e6, step=1e5, format="%.1e")
    ...
    for j in range(Ny):
        for i in range(Nx):
            ...
            if i == 0:  # Cathode (x = 0)
                ...
                A[idx, idx] = 1
                A[idx, idx - 1] = -1
                b[idx] = (j_anode * 1e6) / kappa[j, i] * dx  # Scale current density
...
def j_butler_volmer(phi):
    eta = phi - phi_eq / phi_0
    return j0 * (np.exp(alpha * eta) - np.exp(-(1 - alpha) * eta))
...
def save_vts(c, phi, epsilon_xx, epsilon_yy, sigma_xx, sigma_yy, sigma_vm, t, index):
...
# Save to HDF5
def save_h5(c, phi, epsilon_xx, epsilon_yy, sigma_xx, sigma_yy, sigma_vm, t):
...
def plot_pyvista(c, phi, epsilon_xx, epsilon_yy, sigma_xx, sigma_yy, sigma_vm, t):
    ...
    if t_max_dim < min_t_max_dim:
        ...
        with st.spinner("Running simulation..."):
            ...
            for i, t in enumerate(t_eval):
                ...
                save_vts(vars["c"], vars["phi"], vars["epsilon_xx"], vars["epsilon_yy"], vars["sigma_xx"], vars["sigma_yy"], vars["sigma_vm"], t, i)
                save_h5(vars["c"], vars["phi"], vars["epsilon_xx"], vars["epsilon_yy"], vars["sigma_xx"], vars["sigma_yy"], vars["sigma_vm"], t)
                plot_pyvista(vars["c"], vars["phi"], vars["epsilon_xx"], vars["epsilon_yy"], vars["sigma_xx"], vars["sigma_yy"], vars["sigma_vm"], t)
