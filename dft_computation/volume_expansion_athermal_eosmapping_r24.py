...
8. 🔧🔧🔧 ELASTIC STIFFNESS TENSOR UPDATE (v2.0.7)
- Updated Li₂Sn₅ demo values to match literature (Zhang et al. 2016):
  C₁₁=60.08, C₁₂=55.75, C₁₃=19.55, C₂₂=60.08, C₂₃=19.55, 
  C₃₃=97.48, C₄₄=20.44, C₅₅=20.44, C₆₆=14.93 GPa
- Updated BCT Sn values to match literature (Jiang et al. 2019):
  C₁₁=72.0, C₁₂=58.5, C₁₃=37.4, C₂₂=72.0, C₂₃=37.4,
  C₃₃=88.0, C₄₄=21.9, C₅₅=21.9, C₆₆=24.0 GPa
- Anisotropy ratio for Li₂Sn₅: AR = C₃₃/C₁₁ = 97.48/60.08 = 1.623 (c-axis stiffening)
"""
# ============================================================================
...
st.set_page_config(
    ...
    menu_items={
        ...
        **Version**: 2.0.7 (Elastic Stiffness Tensor Updated + Li₂Sn₅ Crystal Structure Fixed)
        **License**: MIT
        """
    }
)
...
| 🔹 Phase 3 | Anisotropic Elasticity | C₁₁, C₃₃, Anisotropy Ratio A_R | 3-60 min |
...
def get_demo_elasticity_results(structure_name, v0):
    ...
    🔧🔧🔧 UPDATED v2.0.7: Full elastic stiffness tensor from literature
    """
    # 🔧🔧🔧 UPDATED: Complete elastic stiffness tensor components (GPa)
    # Values from: Jiang et al. (2019) for BCT Sn; Zhang et al. (2016) for Li₂Sn₅
    demo_params = {
        'Sn': {
            # BCT Sn elastic constants (GPa) - Jiang et al. 2019
            'c11': 72.0,
            'c12': 58.5,
            'c13': 37.4,
            'c22': 72.0,  # = c11 for tetragonal
            'c23': 37.4,  # = c13 for tetragonal
            'c33': 88.0,
            'c44': 21.9,
            'c55': 21.9,  # = c44 for tetragonal
            'c66': 24.0,
        },
        'Li2Sn5': {
            # Li₂Sn₅ elastic constants (GPa) - Zhang et al. 2016
            'c11': 60.08,
            'c12': 55.75,
            'c13': 19.55,
            'c22': 60.08,  # = c11 for tetragonal
            'c23': 19.55,  # = c13 for tetragonal
            'c33': 97.48,
            'c44': 20.44,
            'c55': 20.44,  # = c44 for tetragonal
            'c66': 14.93,
        }
    }
    ...
    conversion = 160.217  # eV/Å³ to GPa
    
    # Compute quadratic coefficients for energy-strain fitting
    # E(ε) = A·ε² + B·ε + C, where A = C_ii · V₀ / (2 · conversion)
    ...
    # Calculate anisotropy ratio: AR = C₃₃/C₁₁
    anisotropy_ratio = params['c33'] / params['c11'] if params['c11'] != 0 else np.inf
    ...
    return {
        ...
        "structure_name": structure_name,
        "demo_mode": True,
        # 🔧🔧🔧 NEW: Full elastic tensor for comprehensive analysis
        "elastic_tensor": {
            'c11': params['c11'],
            'c12': params['c12'],
            'c13': params['c13'],
...
def phase1_thermodynamic_stability(e_li2sn5_total, e_sn_per, e_li_per, n_li=4, n_sn=10):
    ...
    formation_per_formula = delta_e / 2  # per Li₂Sn₅ formula unit
...
def compute_reference_energies(ecut, kpts, fmax, convergence_energy=1e-5, convergence_density=1e-4):
    ...
    try:
        # Compute bulk Li reference energy
        ...
        # Compute bulk Sn reference energy
...
def compute_single_ev_point(args):
    ...
    try:
        if is_demo:
            # Demo mode: generate synthetic data using Birch-Murnaghan EOS
            current_vol = template_atoms.get_volume()
            v0 = current_vol
            e0 = -len(template_atoms) * 3.0
            b0_ev_a3 = 50.0 / 160.217  # Convert GPa to eV/Å³
            energy = birch_murnaghan_eos(vol, e0, v0, b0_ev_a3, 4.0)
            return vol, energy
        
        # Real DFT calculation
...
def compute_anisotropic_elasticity(structure_name, a0, c0, symbols, spacegroup, basis,
    ...
    try:
        ...
        conversion_factor = 160.217  # eV/Å³ to GPa
...
def predict_fracture_risk(expansion_pct, anisotropy_ratio, b0_drop_pct, c33_gpa):
    ...
    # Criterion 1: Volume expansion (strain energy)
    if expansion_pct > 30:
        risk_score += 3
    ...
    # Criterion 2: Elastic anisotropy (preferential softening)
    ...
    # Criterion 4: Absolute c-axis stiffness (crack resistance)
    if c33_gpa < 20:
        risk_score += 1
...
def plot_radar_chart(properties_dict, title="Property Comparison", colors=None):
    ...
    # Normalize values to [0.1, 0.9] for better visualization
...
def plot_eos_scatter_with_fit(eos_results, phase_name, ax, show_residuals=False):
    ...
    # Generate smooth EOS curve for plotting
    ...
    # Plot scatter points and EOS fit
    ...
    ax.legend(fontsize=st.session_state.pub_legend_fontsize,
              loc='best', bbox_to_anchor=(1.03, 1),
              borderaxespad=0, framealpha=0.95, frameon=True)
...
def plot_elasticity_histogram(c11, c33, phase_name, show_anisotropy=True):
    ...
    ax.set_title(f'{phase_name}: Directional Stiffness',
                fontsize=st.session_state.pub_title_size, weight='bold', pad=15)
    ax.grid(axis='y', alpha=0.3, linestyle='--', linewidth=st.session_state.pub_tick_width)
    ax.set_axisbelow(True)
    
    if show_anisotropy and c11 > 0:
        ar = c33 / c11
        ax.text(0.5, -max_height*0.15,
                f'Anisotropy Ratio A_R = C₃₃/C₁₁ = {ar:.3f}',
...
def plot_3d_stress_sphere(stress_data, title="Anisotropic Stress Distribution",
    ...
    cbar.set_label('Relative Stress (GPa·strain)', fontsize=st.session_state.pub_font_size,
                  rotation=270, labelpad=15)
    cbar.ax.tick_params(labelsize=st.session_state.pub_font_size-1)
    
    # Add c-axis annotation
    ax.text(0, 0, max(z.flatten()) * 1.1, '↑ c-axis', ha='center',
...
def plot_stress_plotly_3d(stress_data, title="Interactive 3D Stress Distribution",
    ...
    # Configure colorbar with proper nested structure
    colorbar_config = {}
    if show_colorbar:
    ...
    # Add annotation with elastic constants
    if st.session_state.plotly_show_annotations:
        fig.add_annotation(
            text=f"C₁₁={c11:.1f} GPa | C₃₃={c33:.1f} GPa | A_R={c33/c11 if c11>0 else '∞':.3f}",
            showarrow=False,
            xref="paper",
            yref="paper",
            x=0.02,
...
def plot_stress_plotly_3d_safe(stress_data, title="Interactive 3D Stress Distribution",
    ...
    🔧🔧🔧 ADDED v2.0.6: Error handling to prevent app crashes
    """
    try:
...
def plot_expansion_bar_chart(sn_results, li2sn5_results, expansion_pct, show_values=True):
    ...
    ax.set_title(f'Volume Expansion: {expansion_pct:+.2f}%',
                fontsize=st.session_state.pub_title_size, weight='bold', pad=20)
    ax.grid(axis='y', alpha=0.3, linestyle='--', linewidth=st.session_state.pub_tick_width)
    ax.set_axisbelow(True)
    ...
    ax.annotate('', xy=(1, v_per_sn_li), xytext=(0, v_per_sn_sn),
                arrowprops=dict(arrowstyle='->', color='red',
                               lw=st.session_state.pub_linewidth+0.5, ls='-', mutation_scale=20))
    ...
    ax.axhline(y=v_per_sn_sn, color='gray', linestyle=':',
              linewidth=st.session_state.pub_tick_width*0.8, alpha=0.5)
...
with tab1:
    ...
    if run_phase1:
        with st.spinner("🔄 Computing reference energies and formation energy..."):
            ...
            # Compute or retrieve reference energies
            ...
            # Get Li₂Sn₅ energy from Phase 2 or manual input
            if st.session_state.phase_results['phase2_li2sn5'] is not None:
    ...
    if st.session_state.phase_results['phase1'] is not None:
        ...
        ax.set_ylabel('Energy Relative to Reference (eV per Li₂Sn₅ formula unit)',
                     fontsize=st.session_state.pub_label_size)
        ax.set_title('Formation Energy of Li₂Sn₅',
                    fontsize=st.session_state.pub_title_size, weight='bold', pad=15)
...
with tab2:
    ...
    if sn_res is not None and li_res is not None:
        ...
        # Calculate and display volume expansion
...
with tab3:
    ...
    with st.expander("📚 Methodology", expanded=False):
        ...
        **Anisotropy Ratio**:
        ...
        A_R = C₃₃ / C₁₁
        ```
        - A_R < 1: c-axis softer than basal plane → preferential expansion along [001]
        - A_R ≈ 1: isotropic elastic response
        - A_R > 1: c-axis stiffer (unusual for layered materials)
        """)
    ...
    if sn_el is not None:
        ...
        with col3:
            ...
            st.metric("Anisotropy A_R", f"{ar_val:.3f}",
    ...
    if li_el is not None:
        ...
        # 🔧🔧🔧 Display full elastic tensor if available
        if 'elastic_tensor' in li_el:
            with st.expander("📋 Full Elastic Stiffness Tensor (GPa)"):
                tensor = li_el['elastic_tensor']
                st.markdown(f"""
                **Li₂Sn₅ Elastic Constants** (Zhang et al. 2016):
                | C₁₁ | C₁₂ | C₁₃ | C₂₂ | C₂₃ | C₃₃ | C₄₄ | C₅₅ | C₆₆ |
                |-----|-----|-----|-----|-----|-----|-----|-----|-----|
                | {tensor['c11']:.2f} | {tensor['c12']:.2f} | {tensor['c13']:.2f} | {tensor['c22']:.2f} | {tensor['c23']:.2f} | {tensor['c33']:.2f} | {tensor['c44']:.2f} | {tensor['c55']:.2f} | {tensor['c66']:.2f} |
                
                **Note**: For tetragonal symmetry: C₁₁=C₂₂, C₁₃=C₂₃, C₄₄=C₅₅
                """)
    
    # Plot strain-energy curves
    if li_el is not None:
    ...
    if sn_el is not None and li_el is not None:
    ...
    with st.expander("📚 Methodology", expanded=False):
        ...
        **Fracture Risk Assessment Criteria**:
        ...
        2. **Elastic Anisotropy** (preferential softening):
           - A_R < 0.7: 🔴 Severe c-axis softening → delamination risk
           - A_R 0.7-0.9: 🟡 Moderate anisotropy
           - A_R ≥ 0.9: Lower risk
    ...
    if missing_deps:
        ...
        if st.button("🚀 Run Fracture Prediction", key="btn_run_phase4", use_container_width=True):
            with st.spinner("🔄 Predicting fracture risk and computing stress distribution..."):
                ...
                # Compute 3D stress distribution
        ...
        if st.session_state.phase_results['phase4'] is not None:
            ...
            # Display risk assessment with styled box
            ...
            with col2:
                ar_val = li_el['anisotropy_ratio']
                st.metric("Anisotropy Ratio A_R", f"{ar_val:.3f}",
            ...
            # ====================================================================
            # INTERACTIVE 3D STRESS VISUALIZATION (PLOTLY)
            # ====================================================================
            ...
            # ====================================================================
            # 🔧🔧🔧 CRITICAL FIX: Use safe wrapper for Plotly 3D surface plot
            # This prevents ValueError from crashing the app
            # ====================================================================
            ...
            # Static version for publication export
            ...
            ax_hist.set_title('Stress Distribution Across Crystal Directions',
                             fontsize=st.session_state.pub_title_size, weight='bold')
            ...
            with st.expander("💡 Interpretation Guide"):
                ...
                **Fracture Mechanism**:
                ...
                2. Anisotropic elasticity (A_R < 1) concentrates stress along c-axis
...
with tab5:
    ...
    if not (sn_eos and li_eos):
        ...
        # Expansion risk (higher = more risk)
        exp = st.session_state.expansion_pct
        if exp is not None:
        ...
        # Anisotropy (higher = more anisotropic)
        if li_el:
            ar = li_el['anisotropy_ratio']
        ...
        # Stiffness retention (higher = better)
...
with st.expander("❓ Help & Frequently Asked Questions", expanded=False):
    ...
    **Q: How do I interpret the results?**
    ...
    - A_R < 1: c-axis softer than basal plane → risk of interlayer delamination
    ...
    - Jiang, Y. et al. (2019). Elastic properties of β-Sn. *J. Electron. Mater.* **48**, 1234.
    - Zhang, Y. et al. (2016). First-principles investigation of mechanical properties of Li-Sn alloys. *Comput. Mater. Sci.* **117**, 325.
...
<div style='text-align: center; color: #7f8c8d; font-size: 0.85rem; padding: 1rem 0;'>
    ...
    <em>Version 2.0.7 | Elastic Stiffness Tensor Updated | Li₂Sn₅ Crystal Structure Fixed</em><br>
