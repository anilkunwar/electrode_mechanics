...
# --- Available colormaps for matplotlib heatmap
available_cmaps = sorted([
    m for m in plt.colormaps()
    if not m.endswith("_r")  # remove reversed versions to shorten list
])
...
colormap = st.selectbox("Matplotlib Colormap", available_cmaps)
...
if st.button("Compute Attention and Relevance"):
    try:
        ...
        fig_plotly = go.Figure(data=go.Heatmap(
            ...
            y=token_strs,
            colorscale="Viridis",
            colorbar=dict(title="Attention Weight", tickfont=dict(size=fontsize))
        ))
        ...
        fig_matplotlib = plot_matplotlib_heatmap(attn_matrix, token_strs, cmap=colormap, fontsize=fontsize)
