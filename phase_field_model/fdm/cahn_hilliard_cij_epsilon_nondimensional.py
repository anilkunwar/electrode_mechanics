...
st.sidebar.header("Elastic Material Model")
material_model = st.sidebar.selectbox("Select Material Model", ["Isotropic", "Orthotropic", "Anisotropic"])

# Elastic parameters for Li₂Sn₅
st.sidebar.subheader("Li₂Sn₅ Elastic Parameters")
if material_model == "Isotropic":
    E_Li2Sn5 = st.sidebar.number_input("Li₂Sn₅ Young's Modulus E (Pa)", min_value=1e8, max_value=1e10, value=1e9, step=1e8, format="%.1e")
    nu_Li2Sn5 = st.sidebar.number_input("Li₂Sn₅ Poisson's Ratio ν", min_value=0.0, max_value=0.49, value=0.3, step=0.05)
    C_11_Li2Sn5 = C_22_Li2Sn5 = E_Li2Sn5 / (1 - nu_Li2Sn5**2)
    C_12_Li2Sn5 = C_21_Li2Sn5 = E_Li2Sn5 * nu_Li2Sn5 / (1 - nu_Li2Sn5**2)
elif material_model == "Orthotropic":
    ...
    C_21_Li2Sn5 = C_12_Li2Sn5
else:  # Anisotropic
    C_11_Li2Sn5 = st.sidebar.number_input("Li₂Sn₅ C_11 (Pa)", min_value=1e8, max_value=1e10, value=1e9, step=1e8, format="%.1e")
    C_22_Li2Sn5 = st.sidebar.number_input("Li₂Sn₅ C_22 (Pa)", min_value=1e8, max_value=1e10, value=1e9, step=1e8, format="%.1e")
    C_12_Li2Sn5 = st.sidebar.number_input("Li₂Sn₅ C_12 (Pa)", min_value=0.0, max_value=1e10, value=0.3e9, step=1e8, format="%.1e")
    C_21_Li2Sn5 = st.sidebar.number_input("Li₂Sn₅ C_21 (Pa)", min_value=0.0, max_value=1e10, value=0.4e9, step=1e8, format="%.1e")

# Elastic parameters for Sn
st.sidebar.subheader("BCT Sn Elastic Parameters")
if material_model == "Isotropic":
    E_Sn = st.sidebar.number_input("Sn Young's Modulus E (Pa)", min_value=1e8, max_value=1e10, value=0.5e9, step=1e8, format="%.1e")
    nu_Sn = st.sidebar.number_input("Sn Poisson's Ratio ν", min_value=0.0, max_value=0.49, value=0.33, step=0.05)
    ...
    C_22_Sn = st.sidebar.number_input("Sn C_22 (Pa)", min_value=1e8, max_value=1e10, value=0.5e9, step=1e8, format="%.1e")
    C_12_Sn = st.sidebar.number_input("Sn C_12 (Pa)", min_value=0.0, max_value=1e10, value=0.15e9, step=1e8, format="%.1e")
    C_21_Sn = st.sidebar.number_input("Sn C_21 (Pa)", min_value=0.0, max_value=1e10, value=0.2e9, step=1e8, format="%.1e")

# Colormap selection for each variable
st.sidebar.header("Visualization Settings")
colormaps = {
    "Concentration": st.sidebar.selectbox("Concentration Colormap", colormap_options, index=colormap_options.index("viridis")),
    "Potential": st.sidebar.selectbox("Potential Colormap", colormap_options, index=colormap_options.index("jet")),
    "Strain_xx": st.sidebar.selectbox("Strain_xx Colormap", colormap_options, index=colormap_options.index("rainbow")),
    "Strain_yy": st.sidebar.selectbox("Strain_yy Colormap", colormap_options, index=colormap_options.index("hot")),
    "Stress_xx": st.sidebar.selectbox("Stress_xx Colormap", colormap_options, index=colormap_options.index("coolwarm")),
    "Stress_yy": st.sidebar.selectbox("Stress_yy Colormap", colormap_options, index=colormap_options.index("plasma")),
    "Von_Mises_Stress": st.sidebar.selectbox("Von Mises Stress Colormap", colormap_options, index=colormap_options.index("magma")),
    "Laplacian_Potential": st.sidebar.selectbox("Laplacian Potential Colormap", colormap_options, index=colormap_options.index("seismic"))
}
...
def f_elastic(c):
    hc = h(c)
    C_11 = hc * C_11_Li2Sn5 + (1 - hc) * C_11_Sn
    C_12 = hc * C_12_Li2Sn5 + (1 - hc) * C_12_Sn
    C_21 = hc * C_21_Li2Sn5 + (1 - hc) * C_21_Sn
    C_22 = hc * C_22_Li2Sn5 + (1 - hc) * C_22_Sn
    epsilon_xx = alpha_eigenstrain * beta_2D * (1 - c)
...
def df_elastic_dc(c):
    ...
    C_22 = hc * C_22_Li2Sn5 + (1 - hc) * C_22_Sn
    epsilon_xx = alpha_eigenstrain * beta_2D * (1 - c)
    return - (C_11 + C_12 + C_21 + C_22) * epsilon_xx * alpha_eigenstrain * beta_2D
    ...
    for j in range(Ny):
        for i in range(Nx):
            ...
            if i == 0:  # Cathode (x = 0)
                ...
                A[idx, idx] = 1
                b[idx] = phi_anode / phi_0  # Dirichlet: user-defined voltage
...
def compute_variables(c, phi):
    ...
    C_22 = hc * C_22_Li2Sn5 + (1 - hc) * C_22_Sn
    epsilon_xx = alpha_eigenstrain * beta_2D * (1 - c)
    epsilon_yy = alpha_eigenstrain * beta_2D * (1 - c)  # Isotropic expansion
    sigma_xx = (C_11 + C_12) * epsilon_xx
    sigma_yy = (C_21 + C_22) * epsilon_yy
    sigma_vm = np.sqrt(sigma_xx**2 + sigma_yy**2 - sigma_xx * sigma_yy)  # Von Mises stress
...
def plot_matplotlib(data, title, label, filename, colormap_key, x_scale=Lx, y_scale=Ly):
    plt.figure()
    plt.imshow(data, extent=[0, x_scale*1e6, 0, y_scale*1e6], origin="lower", cmap=colormaps[colormap_key])
...
def plot_pyvista(c, phi, epsilon_xx, epsilon_yy, sigma_xx, sigma_yy, sigma_vm, laplacian_phi, t):
    ...
    try:
        ...
        plt.figure()
        plt.imshow(c, extent=[0, Lx*1e6, 0, Ly*1e6], origin="lower", cmap=colormaps["Concentration"])
    ...
    if t_max_dim < min_t_max_dim:
        ...
        with st.spinner("Running simulation..."):
            ...
            for i, t in enumerate(t_eval):
                ...
                plot_matplotlib(vars["c"], f"Concentration at t = {t*tau:.1e} s", "Sn Mole Fraction", f"conc_t_{t*tau:.1e}.png", "Concentration")
                plot_plotly(vars["phi"], f"Potential at t = {t*tau:.1e} s", "Potential (V)")
                plot_matplotlib(vars["phi"], f"Potential at t = {t*tau:.1e} s", "Potential (V)", f"phi_t_{t*tau:.1e}.png", "Potential")
                plot_plotly(vars["epsilon_xx"], f"Strain_xx at t = {t*tau:.1e} s", "Strain_xx")
                plot_matplotlib(vars["epsilon_xx"], f"Strain_xx at t = {t*tau:.1e} s", "Strain_xx", f"strain_xx_t_{t*tau:.1e}.png", "Strain_xx")
                plot_plotly(vars["epsilon_yy"], f"Strain_yy at t = {t*tau:.1e} s", "Strain_yy")
                plot_matplotlib(vars["epsilon_yy"], f"Strain_yy at t = {t*tau:.1e} s", "Strain_yy", f"strain_yy_t_{t*tau:.1e}.png", "Strain_yy")
                plot_plotly(vars["sigma_xx"], f"Stress_xx at t = {t*tau:.1e} s", "Stress_xx (Pa)")
                plot_matplotlib(vars["sigma_xx"], f"Stress_xx at t = {t*tau:.1e} s", "Stress_xx (Pa)", f"stress_xx_t_{t*tau:.1e}.png", "Stress_xx")
                plot_plotly(vars["sigma_yy"], f"Stress_yy at t = {t*tau:.1e} s", "Stress_yy (Pa)")
                plot_matplotlib(vars["sigma_yy"], f"Stress_yy at t = {t*tau:.1e} s", "Stress_yy (Pa)", f"stress_yy_t_{t*tau:.1e}.png", "Stress_yy")
                plot_plotly(vars["sigma_vm"], f"Von Mises Stress at t = {t*tau:.1e} s", "Von Mises Stress (Pa)")
                plot_matplotlib(vars["sigma_vm"], f"Von Mises Stress at t = {t*tau:.1e} s", "Von Mises Stress (Pa)", f"sigma_vm_t_{t*tau:.1e}.png", "Von_Mises_Stress")
                plot_plotly(vars["laplacian_phi"], f"Laplacian of Potential at t = {t*tau:.1e} s", "Laplacian Potential (V/m²)")
                plot_matplotlib(vars["laplacian_phi"], f"Laplacian of Potential at t = {t*tau:.1e} s", "Laplacian Potential (V/m²)", f"laplacian_phi_t_{t*tau:.1e}.png", "Laplacian_Potential")
