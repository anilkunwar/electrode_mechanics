...
8. 🔧🔧🔧 UPDATED ELASTIC STIFFNESS TENSOR VALUES
- Incorporated full C_ij components from Jiang et al. (2019) & Zhang et al. (2016)
- Corrected demo mode fallback values for C₁₁ and C₃₃ to match literature table
"""
# ============================================================================
...
COLORMAPS_MATPLOTLIB = {
    ...
    'magma': 'magma', 'cividis': 'cividis', 'rocket': 'rocket',
    'mako': 'mako', 'flare': 'flare', 'crest': 'crest', 'icefire': 'icefire',
    'vlag': 'vlag', 'flare_r': 'flare_r', 'rocket_r': 'rocket_r',
    'mako_r': 'mako_r', 'icefire_r': 'icefire_r', 'crest_r': 'crest_r', 'vlag_r': 'vlag_r',
    # Sequential - Traditional
    'Blues': 'Blues', 'BuGn': 'BuGn', 'BuPu': 'BuPu', 'GnBu': 'GnBu',
    'Greens': 'Greens', 'Greys': 'Greys', 'Oranges': 'Oranges',
    'OrRd': 'OrRd', 'PuBu': 'PuBu', 'PuBuGn': 'PuBuGn', 'PuRd': 'PuRd',
    'Purples': 'Purples', 'RdPu': 'RdPu', 'Reds': 'Reds', 'YlGn': 'YlGn',
    'YlGnBu': 'YlGnBu', 'YlOrBr': 'YlOrBr', 'YlOrRd': 'YlOrRd',
    ...
    'seismic': 'seismic', 'Spectral_r': 'Spectral_r', 'RdBu_r': 'RdBu_r',
    'RdYlBu_r': 'RdYlBu_r', 'coolwarm_r': 'coolwarm_r', 'seismic_r': 'seismic_r',
    ...
    'afmhot': 'afmhot', 'gray': 'gray', 'binary': 'binary',
...
**Version**: 2.0.6 (Li₂Sn₅ Crystal Structure Manually Fixed + Plotly Colorbar Fixed + Elastic Tensor Updated)
**License**: MIT
"""
    }
)
...
volume_range = st.sidebar.slider(
    "Volume Scaling Range (×V₀)", min_value=0.80, max_value=1.20,
    value=(0.92, 1.08), step=0.02,
...
strain_range = st.sidebar.slider(
    "Uniaxial Strain Range (%)", min_value=-5.0, max_value=5.0,
    value=(-2.0, 2.0), step=0.5,
...
enable_parallel = st.sidebar.checkbox("Enable Parallel Computation", value=False,
    help="Use multiprocessing for parallel E-V point evaluation. Disable for debugging or single-core environments."
)
n_workers = st.sidebar.slider("Parallel Workers", min_value=1, max_value=max(1, mp.cpu_count()),
...
with st.sidebar.expander("📐 Typography & Layout", expanded=True):
    ...
    st.session_state.pub_font_size = st.slider(
        "Base Font Size (pt)", min_value=8, max_value=20,
        value=st.session_state.pub_font_size, help="Recommended: 10-12pt for most journals"
    )
    st.session_state.pub_title_size = st.slider("Title Font Size (pt)", min_value=10, max_value=25,
        value=st.session_state.pub_title_size, help="Should be larger than base font size")
    st.session_state.pub_label_size = st.slider("Axis Label Size (pt)", min_value=9, max_value=25,
        value=st.session_state.pub_label_size, help="Axis labels (X, Y, Z titles)")
    st.session_state.pub_legend_fontsize = st.slider("Legend Font Size (pt)", min_value=7, max_value=20,
        value=st.session_state.pub_legend_fontsize, help="Legend text size")

with st.sidebar.expander("🖊️ Line & Marker Styling", expanded=True):
    st.session_state.pub_linewidth = st.slider("Line Width (pt)", min_value=0.5, max_value=5.0,
        value=st.session_state.pub_linewidth, step=0.1,
        help="Thickness of plot lines. Recommended: 1.5-2.0pt for publications")
    st.session_state.pub_marker_size = st.slider("Marker Size (pt)", min_value=3, max_value=20,
        value=st.session_state.pub_marker_size, help="Size of scatter plot markers")
    st.session_state.pub_tick_width = st.slider("Tick Mark Width (pt)", min_value=0.5, max_value=5.0,
        value=st.session_state.pub_tick_width, step=0.1, help="Width of axis tick marks")
    st.session_state.pub_box_linewidth = st.slider("Axis Box Width (pt)", min_value=0.5, max_value=5.0,
        value=st.session_state.pub_box_linewidth, step=0.1, help="Width of axis border/box")
...
with st.sidebar.expander("📤 Export Settings", expanded=True):
    ...
    st.session_state.export_format = st.radio(
        "Vector Format", options=['PNG', 'SVG', 'PDF'], horizontal=True
    )
...
def safe_json_serialize(obj):
    ...
    elif isinstance(obj, dict): return {k: safe_json_serialize(v) for k, v in obj.items()}
    elif isinstance(obj, (list, tuple)): return [safe_json_serialize(item) for item in obj]
    else: return str(obj)
...
def validate_eos_results(results, phase_name):
    ...
    if not results: return False, f"{phase_name}: No results object"
...
def create_li2sn5_manual(a=10.35, c=3.15):
    ...
    li_positions = [(0.33, 0.17, 0.5), (0.17, 0.33, 0.5), (0.67, 0.83, 0.5), (0.83, 0.67, 0.5)]
    symbols = ['Sn'] * 10 + ['Li'] * 4
...
def get_demo_ev_results(structure_name, v0_init, n_points, volume_range):
    ...
    if structure_name not in demo_params: return None
    ...
    return {
        "volumes": volumes, "energies": energies, "v0_init": v0_init, "v0_fit": params['v0'],
        "e0_fit": params['e0'], "B0_GPa": params['b0'], "Bp": params['bp'],
        "num_sn": 4 if structure_name == 'Sn (BCT)' else 10, "eos": None, "gp_model": None,
        "computation_time": 0.1, "n_points_computed": n_points, "n_points_requested": n_points,
...
def get_demo_elasticity_results(structure_name, v0):
    ...
    Updated with literature-validated full elastic stiffness tensor components (GPa)
    from Jiang et al. (2019) for BCT Sn and Zhang et al. (2016) for Li2Sn5.
    """
    demo_params = {
        'Sn': {
            'c11': 72.0, 'c12': 58.5, 'c13': 37.4,
            'c22': 72.0, 'c23': 37.4, 'c33': 88.0,
            'c44': 21.9, 'c55': 21.9, 'c66': 24.0,
        },
        'Li2Sn5': {
            'c11': 60.08, 'c12': 55.75, 'c13': 19.55,
            'c22': 60.08, 'c23': 19.55, 'c33': 97.48,
    ...
    return {
        ...
        "demo_mode": True,
        # Full stiffness tensor for reference and future UI expansion
        "Cij_tensor_gpa": {
            "C11": params['c11'], "C12": params['c12'], "C13": params['c13'],
            "C22": params['c22'], "C23": params['c23'], "C33": params['c33'],
...
def compute_reference_energies(ecut, kpts, fmax, convergence_energy=1e-5, convergence_density=1e-4):
    ...
    if not GPAW_AVAILABLE:
        ...
        result = {"e_li_per_atom": -1.908, "e_sn_per_atom": -3.152, "source": "Materials Project + literature benchmark (PBE)", "demo_mode": True}
    ...
    try:
        ...
        li_calc = create_calculator(ecut, kpts=kpts, txt=None, convergence={'energy': convergence_energy, 'density': convergence_density})
        ...
        sn_calc = create_calculator(ecut, kpts=kpts, txt=None, convergence={'energy': convergence_energy, 'density': convergence_density})
        ...
        return {"e_li_per_atom": -1.908, "e_sn_per_atom": -3.152, "source": "Fallback: literature values (PBE)", "demo_mode": True, "error": str(e)}
...
def compute_ev_curve(structure_name, a_init, c_init, symbols, spacegroup, basis,
    ...
    try:
        if structure_name == 'Sn (BCT)':
            try:
                template = crystal('Sn', basis=[(0,0,0)], spacegroup=141, cellpar=[a_init, a_init, c_init, 90, 90, 90])
    ...
    if len(volumes) >= 4:
        try:
            ...
            v0_fit, e0_fit, B0_gpa, Bp_fit, eos = v0_init, np.min(energies) if len(energies) > 0 else 0, 50.0, 4.0, None
    else:
        log_message(f"Insufficient E-V points ({len(volumes)}) for EOS fitting", "error")
        v0_fit, e0_fit, B0_gpa, Bp_fit, eos = v0_init, np.min(energies) if len(energies) > 0 else 0, None, None, None
...
def compute_anisotropic_elasticity(structure_name, a0, c0, symbols, spacegroup, basis,
    ...
    start_time = time.time()
    try:
        if structure_name == 'Sn': template = create_sn_bct_manual(a=a0, c=c0)
        elif structure_name == 'Li2Sn5': template = create_li2sn5_manual(a=a0, c=c0)
        ...
        if structure_name == 'Li2Sn5': template = create_li2sn5_manual(a=a0, c=c0)
        else: template = create_sn_bct_manual(a=a0, c=c0)
...
def predict_fracture_risk(expansion_pct, anisotropy_ratio, b0_drop_pct, c33_gpa):
    ...
    return {"risk_score": risk_score, "risk_level": risk_level, "description": description,
            "contributing_factors": factors, "criteria": {"expansion_pct": expansion_pct, "anisotropy_ratio": anisotropy_ratio, "b0_drop_pct": b0_drop_pct, "c33_gpa": c33_gpa}}
...
def plot_stress_plotly_3d(stress_data, title="Interactive 3D Stress Distribution", cmap_name=None, elevation=25, azimuth=45, show_colorbar=True, wireframe=False):
    ...
    fig.update_layout(
        ...
        scene=dict(
            xaxis=dict(title=dict(text="X", font=dict(size=20, family="Times New Roman")), tickfont=dict(size=15, family="Times New Roman"), showbackground=True, backgroundcolor="rgba(240,240,240,0.5)"),
...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    ...
    "🧭 Phase 3: Anisotropic Elasticity", "💥 Phase 4: Fracture Prediction", "📈 Integrated Dashboard"
])
    ...
    if run_phase1:
        with st.spinner("🔄 Computing reference energies and formation energy..."):
            ...
            if st.session_state.ref_energies is None:
                st.session_state.ref_energies = compute_reference_energies(ecut, kpts_sn, fmax, convergence_energy, convergence_density)
            ref = st.session_state.ref_energies
            if st.session_state.phase_results['phase2_li2sn5'] is not None: e_li2sn5 = st.session_state.phase_results['phase2_li2sn5']["e0_fit"]
            else: e_li2sn5 = e_li2sn5_manual
...
with tab2:
    ...
    if run_sn and st.session_state.phase_results['phase2_sn'] is None:
        with st.spinner(f"🔄 Computing Sn E-V curve ({n_vol} points)..."):
            ...
            try:
                sn_results = compute_ev_curve(structure_name='Sn (BCT)', a_init=5.83, c_init=3.18, symbols='Sn', spacegroup=141, basis=[(0,0,0)], num_sn=4, kpts=kpts_sn, volume_range=volume_range, n_points=n_vol, fmax=fmax, ecut=ecut, use_surrogate=use_surrogate, convergence_energy=convergence_energy, convergence_density=convergence_density, maxiter=maxiter)
                st.session_state.phase_results['phase2_sn'] = sn_results
                st.session_state.computation_times['phase2_sn'] = time.time() - phase2_sn_start
                st.success(f"✅ Sn E-V curve computed in {format_time(st.session_state.computation_times['phase2_sn'])}")
            except Exception as e:
    ...
    if run_li2sn5 and st.session_state.phase_results['phase2_li2sn5'] is None:
        with st.spinner(f"🔄 Computing Li₂Sn₅ E-V curve ({n_vol} points)..."):
            ...
            try:
                li2sn5_results = compute_ev_curve(structure_name='Li2Sn5', a_init=10.35, c_init=3.15, symbols=['Sn', 'Sn', 'Sn', 'Li'], spacegroup=127, basis=[(0, 0, 0), (0.295, 0.795, 0), (0.16, 0.66, 0), (0.33, 0.17, 0.5)], num_sn=10, kpts=kpts_li2sn5, volume_range=volume_range, n_points=n_vol, fmax=fmax, ecut=ecut, use_surrogate=use_surrogate, convergence_energy=convergence_energy, convergence_density=convergence_density, maxiter=maxiter)
                st.session_state.phase_results['phase2_li2sn5'] = li2sn5_results
                st.session_state.computation_times['phase2_li2sn5'] = time.time() - phase2_li_start
                st.success(f"✅ Li₂Sn₅ E-V curve computed in {format_time(st.session_state.computation_times['phase2_li2sn5'])}")
            except Exception as e:
    ...
    if sn_res is not None and li_res is not None:
        ...
        st.markdown(f"""<div style='background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 1.2rem; border-radius: 0.5rem; text-align: center; font-size: 1.3rem; margin: 1rem 0; box-shadow: 0 4px 6px rgba(0,0,0,0.1)'><strong>Volume Expansion: {expansion_pct:+.2f}%</strong> per Sn atom</div>""", unsafe_allow_html=True)
