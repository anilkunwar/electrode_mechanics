...
4. FULL Li₂Sn₅ E-V computation enabled with complete crystallographic parameters
...
st.set_page_config(
    ...
    menu_items={
        ...
        **Version**: 2.0.3 (Phase 4 Variable Scope Fixed)
        **License**: MIT
        """
    }
)
...
def compute_ev_curve(structure_name, a_init, c_init, symbols, spacegroup, basis, 
    ...
    🔧 EXPANDED: Full Li₂Sn₅ E-V computation with complete crystallographic parameters
    ...
    try:
        if structure_name == 'Sn (BCT)':
            ...
            # Li₂Sn₅: Tetragonal, space group P4/mbm (#127)
            # Wyckoff positions:
            #   Sn1: 2a (0, 1/2, 0)
            #   Li:  4j (0.16, 0.66, 0)
            #   Sn2: 4j (0.295, 0.432, 0)
            # Total: 2 Sn1 + 4 Li + 4 Sn2 = 10 Sn + 4 Li = 14 atoms
...
def plot_eos_scatter_with_fit(eos_results, phase_name, ax, show_residuals=False):
    ...
    if vols is None or energies is None or len(vols) == 0 or len(energies) == 0:
        ax.text(0.5, 0.5, f'⚠️ No data\nfor {phase_name}', ha='center', va='center', 
...
with tab2:
    ...
    if run_li2sn5 and st.session_state.phase_results['phase2_li2sn5'] is None:
        with st.spinner(f"🔄 Computing Li₂Sn₅ E-V curve ({n_vol} points)..."):
            ...
            try:
                # ====================================================================
                # 🔧 FULL Li₂Sn₅ E-V COMPUTATION CALL - EXPANDED PARAMETERS
                # ====================================================================
                li2sn5_results = compute_ev_curve(
                    structure_name='Li2Sn5',
                    a_init=10.274,           # Experimental a-lattice parameter (Å)
                    c_init=3.125,            # Experimental c-lattice parameter (Å)
                    symbols=['Sn', 'Li', 'Sn'],  # Chemical symbols for ASE crystal builder
                    spacegroup=127,          # P4/mbm spacegroup number
                    basis=[
                        (0, 0.5, 0),         # Sn1: Wyckoff 2a position
                        (0.16, 0.66, 0),     # Li:  Wyckoff 4j position  
                        (0.295, 0.432, 0)    # Sn2: Wyckoff 4j position
                    ],
...
<div style='text-align: center; color: #7f8c8d; font-size: 0.85rem; padding: 1rem 0;'>
    ...
    <em>Version 2.0.3 | Phase 4 Variable Scope Fixed | GPAW Mixer Compatible</em><br>
